...
status:
  ...
  - name: CHAINS-GIT_URL
	...
	got, err := TaskMaterials(ctx, objects.NewTaskRunObjectV1Beta1(taskRun))
	if err != nil {
...
func TestTaskMaterials(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			ctx := logtesting.TestContextWithLogger(t)
			mat, err := TaskMaterials(ctx, objects.NewTaskRunObjectV1Beta1(tc.taskRun))
			if err != nil {
				t.Fatalf("Did not expect an error but got %v", err)
			}
...
func TestPipelineMaterials(t *testing.T) {
	...
	ctx := logtesting.TestContextWithLogger(t)
	got, err := PipelineMaterials(ctx, createPro("../../../testdata/v1beta1/pipelinerun1.json"), &slsaconfig.SlsaConfig{DeepInspectionEnabled: false})
	if err != nil {
		t.Error(err)
	}
...
func TestStructuredResultPipelineMaterials(t *testing.T) {
	...
	ctx := logtesting.TestContextWithLogger(t)
	got, err := PipelineMaterials(ctx, createPro("../../../testdata/v1beta1/pipelinerun_structured_results.json"), &slsaconfig.SlsaConfig{DeepInspectionEnabled: false})
	if err != nil {
...
func TestFromPipelineParamsAndResults(t *testing.T) {
	tests := []struct {
		name                     string
		pipelineRunObjectV1Beta1 *objects.PipelineRunObjectV1Beta1
		enableDeepInspection     bool
		want                     []common.ProvenanceMaterial
	}{{
		name: "from results",
		pipelineRunObjectV1Beta1: objects.NewPipelineRunObjectV1Beta1(&v1beta1.PipelineRun{
		...
		pipelineRunObjectV1Beta1: createProWithPipelineParamAndTaskResult(),
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			ctx := logtesting.TestContextWithLogger(t)
			got := FromPipelineParamsAndResults(ctx, tc.pipelineRunObjectV1Beta1, &slsaconfig.SlsaConfig{DeepInspectionEnabled: tc.enableDeepInspection})
...
//nolint:all
func createProWithPipelineParamAndTaskResult() *objects.PipelineRunObjectV1Beta1 {
	pro := objects.NewPipelineRunObjectV1Beta1(&v1beta1.PipelineRun{
		Status: v1beta1.PipelineRunStatus{
			PipelineRunStatusFields: v1beta1.PipelineRunStatusFields{
				PipelineSpec: &v1beta1.PipelineSpec{
					Params: []v1beta1.ParamSpec{{
